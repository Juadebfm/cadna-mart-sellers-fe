...
export default function CreditModal({ transaction, onClose }: Props) {
  return (
    <TransactionModalShell title="Credit details" onClose={onClose}>
      <div className="px-5 pb-5 space-y-4">
        {/* Amount card */}
        <div className="bg-[#E6F8F2] rounded-xl px-4 py-3 text-center">
          <p className="text-[22px] font-bold text-[#00AB72]">
            {transaction.amount}
          </p>
          <p className="text-[11px] text-[#00AB72] uppercase tracking-wider mt-0.5">
            Credit
          </p>
        </div>
        ...
        <div className="space-y-2 text-[13px]">
          {[
            { label: "Reference ID", value: "TXN-8821001", mono: true },
            { label: "Date & time", value: transaction.date },
            { label: "Status", value: "Completed", colour: "text-[#00AB72]" },
            { label: "Order", value: transaction.orderRef },
            {
              label: "Product",
              value: transaction.title.split("—")[1]?.trim() ?? "",
            },
            { label: "Customer", value: "Funmi A." },
            { label: "Delivery method", value: "Own Courier" },
          ].map((row) => (
        ...
        <div className="bg-[#FAFAFF] rounded-lg px-3 py-2.5">
          <p className="text-[12px] text-[#9899A3]">
            Payout released automatically after delivery confirmation.
          </p>
        </div>
        ...
        <div>
          ...
          <TimelineStep label="Order delivered" date={transaction.date} done />
          <TimelineStep
            label="Escrow released"
            date="Automatic · within 24hrs"
            done
          />
          <TimelineStep
